-- ═══════════════════════════════════════════════════════════════════════════
-- ACCESS CONTROL · חבילת התקנה ל-SQL Editor · פרויקט flfemffhswlpgpbvhuvy
--
-- להעתיק את כל הקובץ, להדביק, להריץ פעם אחת. טרנזקציה אחת.
-- בטוח להרצה כפולה. לא נכתבת אף שורת נתונים — סכימה ופונקציות בלבד.
-- להריץ אחרי חבילת pilot-gate-sql-editor.sql (בודקת זאת ב-PRE-CHECK).
--
-- ROLLBACK (רק לפני שנכנסו בקשות/תפקידים אמיתיים):
--   drop function if exists public.admin_list_users();
--   drop function if exists public.admin_list_requests();
--   drop function if exists public.app_is_access_admin(uuid);
--   drop function if exists public.reject_access(uuid,text);
--   drop function if exists public.approve_access(uuid,public.app_role,text,text,uuid);
--   drop function if exists public.unsuspend_user(uuid);
--   drop function if exists public.suspend_user(uuid,text);
--   drop function if exists public.revoke_role(uuid,public.app_role);
--   drop function if exists public.grant_role(uuid,public.app_role);
--   drop function if exists public.app_can_grant(uuid,public.app_role);
--   drop function if exists public.request_access(public.app_role,text);
--   drop function if exists public.whoami();
--   drop function if exists public.app_audit(uuid,uuid,text,jsonb,jsonb);
--   drop function if exists public.app_active_super_admin_count();
--   drop function if exists public.app_is_super_admin(uuid);
--   drop function if exists public.app_roles_of(uuid);
...
--   -- ערכי enum שנוספו ל-app_role (super_admin, viewer) אינם ניתנים להסרה
--   -- ב-Postgres; הם נשארים ואינם מזיקים.
-- אחרי נתוני אמת — אין rollback; מוסיפים מיגרציה מתקנת בלבד.
-- ═══════════════════════════════════════════════════════════════════════════
-- ערכי enum חדשים חייבים לחיות מחוץ לטרנזקציה: Postgres אוסר להשתמש
-- בערך enum שנוסף באותה טרנזקציה. שתי השורות רצות ומתקבעות לבדן, והן
-- בטוחות להרצה כפולה. כל השאר — טרנזקציה אחת אטומית.
...
begin
  ...
  from unnest(array['profiles','user_roles','students','pilot_members']) t
  where not exists (select 1 from pg_class c join pg_namespace n on n.oid=c.relnamespace
                    where n.nspname='public' and c.relname=t and c.relkind='r');
  if v_missing is not null then
    raise exception 'PRE-CHECK FAILED · חסרות טבלאות: % (להריץ קודם את חבילת pilot-gate)', v_missing;
  end if;
  if to_regtype('public.app_role') is null then
    raise exception 'PRE-CHECK FAILED · טיפוס app_role חסר';
  end if;
  if to_regprocedure('public.forbid_change()') is null then
    raise exception 'PRE-CHECK FAILED · forbid_change חסר (חוזה ההגשות אינו מותקן)';
  end if;
...
begin
  if to_regprocedure('public.whoami()') is null
     or to_regprocedure('public.grant_role(uuid,public.app_role)') is null
     or to_regprocedure('public.approve_access(uuid,public.app_role,text,text,uuid)') is null
     or to_regprocedure('public.admin_list_requests()') is null then
    raise exception 'POST-CHECK FAILED · פונקציות חסרות';
  end if;
  select count(*) into n from pg_proc p
  join pg_namespace ns on ns.oid = p.pronamespace
  where ns.nspname='public'
    and p.proname in ('app_roles_of','app_is_super_admin','app_active_super_admin_count',
                      'app_audit','app_can_grant','app_is_access_admin')
    and exists (select 1 from aclexplode(coalesce(p.proacl, acldefault('f', p.proowner))) a
                join pg_roles r on r.oid = a.grantee
                where r.rolname in ('anon','authenticated'));
  if n > 0 then
    raise exception 'POST-CHECK FAILED · % פונקציות עזר עדיין חשופות ללקוחות', n;
  end if;
  if not exists (select 1 from pg_policies where schemaname='public'
                 and tablename='access_requests' and policyname='ar_read') then
    raise exception 'POST-CHECK FAILED · מדיניות ar_read חסרה';
  end if;
  raise notice 'POST-CHECKS PASSED · access control מותקן';
end $do$;
