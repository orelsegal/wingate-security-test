...
const QUICK = [
  { key: 'guide',     icon: '🧾', label: 'מדריך החקר',  desc: 'שלבים וטיפים',  bg: 'bg-blue-50 text-blue-700 border-blue-200' },
  { key: 'rubric',    icon: '🏆', label: 'מחוון הערכה', desc: '100 נקודות',    bg: 'bg-amber-50 text-amber-700 border-amber-200' },
  { key: 'sources',   icon: '📂', label: 'בנק מקורות',  desc: 'חוקים ונתונים', bg: 'bg-green-50 text-green-700 border-green-200' },
  { key: 'portfolio', icon: '📓', label: 'מחברת המסע',  desc: 'כל תשובותיי',   bg: 'bg-purple-50 text-purple-700 border-purple-200' },
];

export default function CivicsDashboard() {
  const { navigate, isDone, pct, stationsDone } = useCivics();
    ...
    <div dir="rtl" className="max-w-6xl mx-auto px-4 py-6 pb-24 lg:pb-8">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">מסע חקר אזרחי · יחידת 30% באזרחות</p>
        </div>
      </div>
      ...
      <div className="card p-5 mb-7">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-sm text-gray-500">{stationsDone}/6 שלבי חקר הושלמו</p>
          </div>
        ...
        {pct === 100 && <p className="mt-3 text-sage-600 font-bold text-sm text-center">🎉 כל הכבוד! השלמתם את מסע החקר כולו!</p>}
      </div>

      {/* Stations */}
      <h2 className="font-bold text-navy mb-3 flex items-center gap-2"><span>⚖️</span> שלבי החקר האזרחי</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-8">
        {CIVICS_STATIONS.map((s, i) => {
          const done  = isDone(`civics-${s.id}`);
          const canGo = s.id === 1 || isDone(`civics-${s.id - 1}`) || done;
          return (
            <button key={s.id} onClick={() => canGo && navigate(`station-${s.id}`)} disabled={!canGo}
              ...
              <p className="text-[10px] font-bold text-terracotta uppercase tracking-wide mb-0.5">שלב {s.id}</p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {QUICK.map(({ key, icon, label, desc, bg }) => (
          <button key={key} onClick={() => navigate('station-1')}
