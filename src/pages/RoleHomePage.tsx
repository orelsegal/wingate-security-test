...
import {
  ...
  Dumbbell, ChevronLeft, TrendingUp, MessageSquare
} from "lucide-react";
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
    {cards.map((card) => (
      <button
        ...
        className={`card-premium p-5 text-start group transition-all duration-300 ${
          card.comingSoon
            ? "opacity-50 cursor-not-allowed"
            : "hover:shadow-[0_8px_30px_-6px_hsl(150,20%,30%,0.14)] hover:-translate-y-1 active:translate-y-0 active:shadow-[var(--shadow-card)] cursor-pointer"
        }`}
      >
        <div className="flex items-start gap-4">
          <div
            className={`w-12 h-12 rounded-2xl ${card.color} flex items-center justify-center shrink-0 transition-transform duration-300 ${
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              {card.comingSoon && (
                <span className="text-[9px] bg-accent px-2 py-0.5 rounded-full text-muted-foreground font-medium">
                  בקרוב
                </span>
              )}
              {card.badge && (
            ...
            <p className="text-[12px] text-muted-foreground mt-1.5 leading-relaxed">{card.description}</p>
          </div>
          {!card.comingSoon && (
            <ChevronLeft
              className="h-4 w-4 text-muted-foreground/20 shrink-0 mt-1 group-hover:text-primary/60 transition-colors duration-200"
              strokeWidth={1.5}
            />
          )}
        </div>
...
const AdminHome = () => {
  ...
  const cards: ActionCard[] = [
    { id: "overview", title: "סקירה כללית", description: "דשבורד עם נתונים, KPIs וסטטיסטיקות", icon: LayoutDashboard, color: "bg-primary/10", iconColor: "text-primary", path: "/dashboard" },
...
const TeacherHome = () => {
  ...
  const cards: ActionCard[] = [
    ...
    { id: "overview", title: "סקירה כללית", description: "דשבורד עם מבט על מצב התלמידים", icon: LayoutDashboard, color: "bg-primary/10", iconColor: "text-primary", path: "/dashboard" },
  ];
  ...
  return (
    <div className="p-5 md:p-10 lg:p-14 max-w-[880px] mx-auto">
      ...
      <section className="mb-8">
        <h1 className="text-[22px] md:text-[28px] font-bold text-foreground tracking-tight">
          שלום, {user?.name} 👋
        </h1>
      ...
      {/* Action cards */}
      <section>
        <h2 className="text-[15px] font-semibold text-primary mb-5">מה תרצה לעשות?</h2>
