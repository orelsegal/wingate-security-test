...
const MathRoadmapPage = () => {
  ...
  // Build scenic roadmap nodes — unified design with literature roadmaps.
  const scenicNodes: ScenicNode[] = useMemo(
    () =>
      UNITS.map((u, i) => {
        const status: ScenicNode["status"] =
          i < activeUnit ? "done" : i === activeUnit ? "current" : "available";
        return {
          id: String(i),
          title: u.title,
          subtitle: `יחידה ${i + 1}`,
          meta: `${u.topics.length} נושאים`,
          status,
        };
      }),
    [activeUnit]
  ...
  return (
    <div className="p-5 md:p-8 lg:p-10 max-w-[1400px] mx-auto" dir="rtl">
      ...
      {/* Unified scenic roadmap (same design across all subjects) */}
      <div className="mb-7">
        <ScenicRoadmap
          nodes={scenicNodes}
          onSelect={handleSelect}
          onContinue={() => handleSelect(Math.min(activeUnit + 1, UNITS.length - 1))}
