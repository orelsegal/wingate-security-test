  ...
  return (
    <div className="p-5 md:p-8 lg:p-10 max-w-[1280px] mx-auto" dir="rtl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-5 mb-6">
        {/* Roadmap visual */}
        <div className="bg-gradient-to-b from-violet-50/50 to-sky-50/30 rounded-3xl border border-border p-5 shadow-[var(--shadow-card)] relative overflow-hidden min-h-[520px]">
          <div className="flex items-center justify-between mb-5">
          ...
          <div className="relative flex flex-col items-center gap-7 py-4">
            <svg className="absolute inset-0 w-full h-full pointer-events-none" preserveAspectRatio="none">
              <path d="M 50% 0 Q 30% 15%, 50% 30% T 50% 60% T 50% 90%" stroke="hsl(270 50% 85%)" strokeWidth="3" strokeDasharray="4 4" fill="none" />
            </svg>
            {nodes.map((n, i) => {
              ...
              return (
                <div key={n.id} className={`relative flex items-center gap-3 z-10 ${offset}`}>
                  ...
                  <div className="bg-white/70 backdrop-blur rounded-xl px-3 py-1.5 border border-border/40 max-w-[180px]">
                  ...
                  {isDone && <div className="absolute -bottom-2 right-3 flex gap-0.5">
                    {[1,2,3].map(s => <Star key={s} className="h-2.5 w-2.5 fill-amber-400 text-amber-400" strokeWidth={0} />)}
                  </div>}
                </div>
              );
            })}
        ...
        <div className="space-y-5">
              ...
              {componentRows.map((c, i) => (
                <div key={i} className="flex items-center justify-between gap-2 py-1.5">
                  <div className="shrink-0">
                    {c.locked ? <Lock className="h-4 w-4 text-muted-foreground/40" strokeWidth={2} /> :
                     c.done ? <CheckCircle2 className="h-5 w-5 text-emerald-500 fill-emerald-50" strokeWidth={2} /> :
                     <div className="w-5 h-5 rounded-full border-2 border-border" />}
                  </div>
                  <div className="flex-1 text-end">
                    <p className="text-[11.5px] font-semibold text-foreground leading-tight">{c.title}</p>
                    <p className="text-[9.5px] text-muted-foreground mt-0.5">{c.sub}</p>
                  </div>
                  <div className="w-8 h-8 rounded-lg bg-muted/40 flex items-center justify-center shrink-0">
                    <c.icon className="h-4 w-4 text-muted-foreground" strokeWidth={2} />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-card rounded-3xl border border-border p-5 shadow-[var(--shadow-card)]">
            ...
            <div className="flex flex-wrap gap-2 justify-end">
              {(coveredTopics.length > 0 ? coveredTopics : (parts[0]?.units[0]?.items.slice(0, 4).map(i => i.title) || [])).slice(0, 6).map((t, i) => (
                <span key={i} className="inline-flex items-center gap-1 bg-violet-50 text-violet-700 text-[10.5px] font-medium rounded-full px-2.5 py-1 border border-violet-100">
                  {t}
                </span>
              ))}
            </div>
