...
// Hardcoded classes for now (could be moved to DB later)
...
function SportsTab() {
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    const { error } = await (supabase.from("sports" as any) as any).insert({ sport_name: newItem.trim() });
    setSaving(false);
...
function SubjectsTab() {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-[10px] text-muted-foreground/60">הערה: מחיקת מקצועות דורשת הרשאות מיוחדות</p>
      {isLoading ? (
        <div className="flex justify-center py-8"><Loader2 className="h-5 w-5 animate-spin text-muted-foreground" /></div>
      ) : subjects.length === 0 ? (
