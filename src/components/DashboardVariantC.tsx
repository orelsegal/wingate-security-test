...
const DashboardVariantC = ({ embedded = false }: Props) => {
  ...
  return (
    <div className={embedded ? "max-w-[1400px]" : "p-5 md:p-8 lg:p-10 max-w-[1400px]"} dir="rtl">
      ...
      {/* ══════════════════════════════════════════
          SECTION 2 — 4 KPI CARDS (unified stats strip)
      ══════════════════════════════════════════ */}
      {!isTeacher && (
        <section className="grid grid-cols-2 xl:grid-cols-4 gap-3 mb-6">
          {/* Card 1 — total students */}
          <div className="card-premium p-4 md:p-5 border-t-2 border-primary/20">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[11px] font-medium text-muted-foreground">ספורטאים</span>
              <Users className="h-3.5 w-3.5 text-muted-foreground/30" strokeWidth={1.5} />
            </div>
            <p className="text-[32px] md:text-[36px] font-semibold text-foreground leading-none tracking-tight font-stat">
              {totalStudents}
            </p>
            <p className="text-[11px] font-medium text-muted-foreground mt-2.5">פעילים במערכת</p>
          </div>

          {/* Card 2 — average score */}
          <div className="card-premium p-4 md:p-5 border-t-2 border-primary/20">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[11px] font-medium text-muted-foreground">ממוצע כללי</span>
              <TrendingUp className="h-3.5 w-3.5 text-muted-foreground/30" strokeWidth={1.5} />
            </div>
            <p className="text-[32px] md:text-[36px] font-semibold text-foreground leading-none tracking-tight font-stat">
              {avgScore}
            </p>
            <p className="text-[11px] font-medium text-muted-foreground mt-2.5">ממוצע משוקלל</p>
          </div>

          {/* Card 3 — on track (green dot) */}
          <div className="card-premium p-4 md:p-5 border-t-2 border-primary/20">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[11px] font-medium text-muted-foreground">במסלול</span>
              <span className="w-2 h-2 rounded-full bg-success/60 ring-1 ring-success/20" />
            </div>
            <p className="text-[32px] md:text-[36px] font-semibold text-foreground leading-none tracking-tight font-stat">
              {greenCount}
            </p>
            <p className="text-[11px] font-medium text-muted-foreground mt-2.5">
          ...
          <div className="card-premium p-4 md:p-5 border-t-2 border-primary/20">
            <div className="flex items-center justify-between mb-3">
              <span className="text-[11px] font-medium text-muted-foreground">דורשים טיפול</span>
      ...
      {!isTeacher && totalStudents > 0 && (
        <section className="card-premium p-4 md:p-5 mb-8 md:mb-10">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-[14px] font-semibold text-foreground">התפלגות סטטוס כולל</h2>
            <span className="text-[11px] text-muted-foreground">{totalStudents} ספורטאים</span>
          </div>

          {/* Progress bar */}
          <div className="flex h-2.5 rounded-full overflow-hidden bg-muted/60 gap-px mb-4">
            {greenPct > 0 && (
              <div className="bg-success/70 rounded-s-full transition-all duration-700" style={{ width: `${greenPct}%` }} />
            )}
            {yellowPct > 0 && (
              <div className="bg-warning/70 transition-all duration-700" style={{ width: `${yellowPct}%` }} />
            )}
            {redPct > 0 && (
              <div className="bg-destructive/70 rounded-e-full transition-all duration-700" style={{ width: `${redPct}%` }} />
            )}
          </div>

          {/* Legend — pill badges */}
          <div className="flex flex-wrap items-center gap-3">
            <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold status-green">
              במסלול
            </span>
            <span className="text-[12px] text-foreground/80 tabular-nums font-normal">
              {greenCount}/{totalStudents}
            </span>

            <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold status-yellow">
              פערים
            </span>
            <span className="text-[12px] text-foreground/80 tabular-nums font-normal">
              {yellowCount}/{totalStudents}
            </span>

            <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold status-red">
              בסיכון
            </span>
            <span className="text-[12px] text-foreground/80 tabular-nums font-normal">
              {redCount}/{totalStudents}
            </span>
          </div>
        </section>
      )}
      ...
      <section className="mb-10 md:mb-12">
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-[12px] text-muted-foreground mt-0.5 font-normal">התפלגות מצב אקדמי לפי ענף ספורט</p>
          </div>
          <button
        ...
        <div className="card-premium overflow-hidden">
          ...
          <div
            className="hidden md:grid px-5 py-3 border-b border-border bg-muted/60 gap-4 items-center"
          ...
          >
            {[
              ...
              <span key={col.label} className={`text-[11px] font-semibold text-muted-foreground uppercase tracking-wide ${col.cls}`}>
                {col.label}
              </span>
            ))}
          </div>
          ...
          {branchStats.length === 0 ? (
            ...
            const zebraClass = idx % 2 === 0 ? "bg-background" : "bg-muted/20";
            return (
              <div key={branch.name} className={`border-b border-border last:border-0 ${zebraClass}`}>
                {/* Mobile row */}
                <div className="md:hidden px-4 py-4">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-[13px] font-semibold text-foreground">{branch.name}</span>
                    <div className="flex items-center gap-3">
                      <span className="text-[13px] font-semibold text-success/80">{branch.green}</span>
                      <span className="text-[13px] font-semibold text-warning/80">{branch.yellow}</span>
                      {branch.red > 0 && <span className="text-[13px] font-semibold text-destructive/80">{branch.red}</span>}
                      <StatusBadge type={branch.overall} />
                    </div>
                  </div>
                  <div className="flex h-2 rounded-full overflow-hidden bg-muted/60 gap-px">
                    {gPct > 0 && <div className="bg-success/65 rounded-s-full" style={{ width: `${gPct}%` }} />}
                    {yPct > 0 && <div className="bg-warning/65" style={{ width: `${yPct}%` }} />}
                    {rPct > 0 && <div className="bg-destructive/65 rounded-e-full" style={{ width: `${rPct}%` }} />}
                  </div>
                </div>
                ...
                >
                  <span className="text-[13px] font-semibold text-foreground">{branch.name}</span>
                  <span className="text-[13px] font-semibold text-foreground text-center tabular-nums">{branch.total}</span>
                  <span className="text-[13px] font-semibold text-success/80 text-center tabular-nums">{branch.green}</span>
                  <span className={`text-[13px] font-semibold text-center tabular-nums ${branch.yellow > 0 ? "text-warning/80" : "text-muted-foreground/20"}`}>
                    {branch.yellow}
                  </span>
                  <span className={`text-[13px] font-semibold text-center tabular-nums ${branch.red > 0 ? "text-destructive/80" : "text-muted-foreground/20"}`}>
                    {branch.red}
                  </span>
                  {/* Progress bar */}
                  <div className="flex h-2 rounded-full overflow-hidden bg-muted/60 gap-px">
                    {gPct > 0 && <div className="bg-success/65 rounded-s-full transition-all" style={{ width: `${gPct}%` }} />}
                    {yPct > 0 && <div className="bg-warning/65 transition-all" style={{ width: `${yPct}%` }} />}
                    {rPct > 0 && <div className="bg-destructive/65 rounded-e-full transition-all" style={{ width: `${rPct}%` }} />}
                  </div>
      ...
      <section>
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-[14px] font-semibold text-foreground">ספורטאים לטיפול</h2>
            <p className="text-[12px] text-muted-foreground mt-0.5 font-normal">
              {statusFilter === "all"
          ...
          {filteredAlerts.length === 0 ? (
            ...
            filteredAlerts.map((alert) => (
              <div
                ...
                className="flex items-center justify-between py-3 px-5 cursor-pointer hover:bg-accent/30 transition-colors duration-100"
              >
                <div className="flex items-center gap-3">
