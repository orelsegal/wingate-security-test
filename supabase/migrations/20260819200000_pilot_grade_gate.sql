-- ═══════════════════════════════════════════════════════════════════════════
-- PILOT · grade gate for the literature 30% loop.
--
-- Ports the grade/unlock design that was built and reviewed in
-- wingate-platform (learning_engine + complete_submission_state_machine)
-- into the live contract, adapted to the entities that actually exist here:
-- learning_tasks instead of units, students.id as the learner identity.
-- One source of truth — nothing here duplicates the platform DB; the design
-- moved, not the data.
--
-- The approved semantics, encoded:
--   · a gate opens only from an AUTHORITATIVE grade at/above the task's
--     threshold (task.pass_threshold, else 85), or a documented manual
--     unlock. A shadow (AI) grade can never open anything.
--   · an unlock is monotonic: nothing updates or deletes it, and a later
--     threshold change cannot revoke it.
--   · the gate applies only to server-side pilot members. Everyone else
--     keeps today's behaviour.
--   · approve_submission's signature is untouched; grading approval is a
--     NEW rpc, grade_and_approve.
-- ═══════════════════════════════════════════════════════════════════════════
